...
describe ("#unit talent composition,", function ( )
    ...
    it ("should merge without conflicts if definitions are identical",
    function ( )
        local talent = talents.compose (
            example.talents.observable,
            example.talents.observable
        )
    ...
    it ("should fail for non implemented requirements cases", function ( )
        ...
        assert.error (procedure, reason.application.required ('react'))
    end)
    ...
    it ("should fail for existent conflicts", function ( )
        local function procedure ( )
            ...
            local _ = talents.decorate (
                talent,
                { observers = weak.key ( ), }
            )
        end

        assert.error (procedure, reason.application.conflict ('notify'))
    end)

    it ("may have conflicts ignored through override with talent inheritance",
    function ( )
        local supertalent = talents.compose (
            example.talents.event,
            example.talents.observable
        )
        ...
        local subtalent = talents.extend (supertalent, {
            notify = function ( )
                error (table.concat ({
                    ...
                    "But I want to throw away these conflicts for now.",
                }, " "))
            end,
        })

        local _ = talents.decorate (subtalent, { observers = weak.key ( ), })

        assert.truthy (talents.provides (subtalent, 'notify'))
    end)
end)
