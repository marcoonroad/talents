...
describe ("talent inheritance,", function ( )
    ...
    it ("should extend another talent", function ( )
        local spatial = talents.extend (plane, {
            ...
            is2D = function ( ) return false end,
            is3D = function ( ) return true  end,
        })

        local point2D = talents.decorate (plane, { x = 1, y = 2, })
        local point3D = talents.decorate (spatial, { x = 1, y = 2, z = 3, })
    ...
    it ("should not observe any direct effects from extension structure",
    function ( )
        local structure = { x = 0, }
