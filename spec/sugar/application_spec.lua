...
describe ("#unit talent application with syntax sugar,", function ( )
    it ("should decorate given object", function ( )
        local object = example.talents.labeled (example.target.point)
    ...
    it ("should fail due non-implemented requirements", function ( )
        local function procedure ( )
            return example.talents.labeled { x = 0, y = 0 }
        end
    ...
    function ( )
        local first  = example.talents.labeled (example.target.point)
        local second = example.talents.labeled (example.target.point)
    ...
    it ("should not cause effects on target object", function ( )
        ...
        local decorated = example.talents.labeled (point)
    ...
    function ( )
        ...
        function metatable.__newindex (_, selector)
            error (final: format (tostring (selector)))
        end

        local object = example.talents.labeled (point (2, 5))
